...
std::vector<Eigen::Vector2d> get_drone_velocity_setpoint(std::vector<Eigen::Vector4d> current_drone_state, std::vector<std::vector<Eigen::Vector2d>> path,std::vector<bool> found){
    ...
    for (int i =0; i< current_drone_state.size(); i++){
        ...
        if (found[i]){
            ...
            drone_setpoint_error[i][0] = drone_setpoint_error_x/std::sqrt(drone_setpoint_error_x);
            drone_setpoint_error[i][1] = drone_setpoint_error_y/std::sqrt(drone_setpoint_error_y);
